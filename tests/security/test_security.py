    ...
    async def test_password_validation(self, client, password, is_valid):
        ...
        if is_valid:
            ...
            assert response.status_code in (
                201, 400, 409, 422), f"Expected 201/400/409/422 for valid password, got {response.status_code}"
            ...
            assert response.status_code in (
                400, 422), f"Expected 400/422 validation error for invalid password, got {response.status_code}"
            ...
            if option_text == xss_input:
                for pattern in dangerous_patterns:
                    if pattern in option_text.lower():
                        # If pattern is present, it should be HTML escaped
                        escaped_pattern = html.escape(pattern)
                        if pattern in option_text and escaped_pattern not in html.escape(
                                option_text):
                            is_sanitized = False
